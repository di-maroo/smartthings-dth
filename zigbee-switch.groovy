...
metadata {
	definition (name: "ZigBee Switch (custom)", namespace: "smartthings", author: "SmartThings", ocfDeviceType: "oic.d.switch", runLocally: true, minHubCoreVersion: '000.019.00012', executeCommandsLocally: true, genericHandler: "Zigbee") {
		capability "Actuator"
...
private Boolean needsDisabling2MinHeartbeat() {
	return (device.getDataValue("manufacturer") == "_TZ3000_9hpxg80k") || (device.getDataValue("manufacturer") == "ORVIBO")
}
...
def configure() {
	...
	zigbee.onOffRefresh() + zigbee.onOffConfig()
    if (needsDisabling2MinHeartbeat()) {
		// some switches (Yagusmart, Orvibo) will send out device anounce message at ervery 2 mins as heart beat,setting 0x0099 to 1 will disable it.
